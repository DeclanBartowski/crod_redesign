...
$APPLICATION->SetTitle("Жюри муниципалитета " . CURRENT_USER_REGION_NAME);
global $USER;
...
<?if(!empty($_REQUEST['ERRORS']) && !empty($_SESSION['ERROR_ALERT'])):?>
	...
	if(!empty($_REQUEST['ERROR_CODE'])){
		$errors[] = urldecode(unserialize($_REQUEST['ERROR_CODE']));
...
<?endif;?>
<?if(!empty($_REQUEST['SUCCESS']) && $_SESSION['SUCCESS_ALERT']):?>
...
<?endif;?>
<a href="index.php">К основному меню личного кабинета</a><br>
<?
